...
//    }
     ...
     * 导出接口
     */
    @GetMapping("/inventoryCheck/exportExcel")
    public void exportExcel(HttpServletResponse response) throws Exception {
        List<WarehouseCountingVO> l = warehouseService.warehouseCounting();
        List<WarehouseExcelVO> list = new ArrayList<>();
        for(int i=0;i<l.size();i++){
            WarehouseExcelVO warehouseExcelVO = new WarehouseExcelVO();
            BeanUtils.copyProperties(l.get(i),warehouseExcelVO);
            warehouseExcelVO.setId(i+1);
            warehouseExcelVO.setPid(l.get(i).getProduct().getId());
            list.add(warehouseExcelVO);
        }
        ExcelWriter writer = ExcelUtil.getWriter(true);
        writer.addHeaderAlias("id","行号");
        writer.addHeaderAlias("pid","商品编号");
        writer.addHeaderAlias("quantity","数量");
        writer.addHeaderAlias("purchasePrice","进价");
        writer.addHeaderAlias("batchId","批次");
        writer.addHeaderAlias("productionDate","出厂日期");
        writer.write(list,true);

        String time = new SimpleDateFormat("yyyyMMdd").format(new Date()).toString();
        String temp = "库存盘点"+time;
    ...
    @GetMapping("/warehouse/queryAll")
    public Response findAllWarehouseId() {
        return Response.buildSuccess(warehouseService.queryAllWarehouseId());
    }
}
